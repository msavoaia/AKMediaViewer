...
public class AKMediaViewerController : UIViewController, UIScrollViewDelegate {
    ...
    override public func viewDidAppear(_ animated: Bool) {
        NotificationCenter.default.addObserver(self, selector: #selector(AKMediaViewerController.orientationDidChangeNotification(_:)), name: UIDevice.orientationDidChangeNotification, object: nil)
    ...
    public func updateOrientationAnimated(_ animated: Bool) {
        ...
        if (UIDevice.current.orientation.isLandscape && previousOrientation.isLandscape) ||
            (UIDevice.current.orientation.isPortrait && previousOrientation.isPortrait)
        {
            duration *= 2
        }
    ...
    @objc func handleDoubleTap(_ gesture: UITapGestureRecognizer) {
        ...
        UIView.animate(withDuration: 0.5, delay: 0, options: UIView.AnimationOptions.beginFromCurrentState, animations: { () -> Void in
