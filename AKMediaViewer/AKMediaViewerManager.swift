...
@objc public protocol AKMediaViewerDelegate : NSObjectProtocol {
    ...
    */
    @objc optional func mediaFocusManager(_ manager: AKMediaViewerManager, imageViewForView view: UIView) -> UIImageView
    ...
    @objc optional func mediaFocusManager(_ manager: AKMediaViewerManager, finalFrameForView view: UIView) -> CGRect
    
    // Called when a focus view is about to be shown. For example, you might use this method to hide the status bar.
    @objc optional func mediaFocusManagerWillAppear(_ manager: AKMediaViewerManager)
    
    // Called when a focus view has been shown.
    @objc optional func mediaFocusManagerDidAppear(_ manager: AKMediaViewerManager)
    ...
    @objc optional func mediaFocusManagerWillDisappear(_ manager: AKMediaViewerManager)
    
    // Called when the view has be dismissed by the 'done' button or by gesture.
    @objc optional func mediaFocusManagerDidDisappear(_ manager: AKMediaViewerManager)
    
    // Called before mediaURLForView to check if image is already on memory.
    @objc optional func mediaFocusManager(_ manager: AKMediaViewerManager, cachedImageForView view: UIView) -> UIImage
}

// MARK: - AKMediaViewerManagerqq
...
public class AKMediaViewerManager : NSObject, UIGestureRecognizerDelegate {
    ...
    public func installOnView(_ view: UIView) {
        ...
        let url: URL = delegate!.mediaFocusManager(self, mediaURLForView: view)
    ...
    func setupAccessoryViewOnFocusViewController(_ focusViewController: AKMediaViewerController!) {
        if(topAccessoryController == nil) {
            ...
            defaultController.doneButton.addTarget(self, action: #selector(AKMediaViewerManager.endFocusing), for:UIControl.Event.touchUpInside)
    ...
    func loadImageFromURL(_ url: URL, onImageView imageView: UIImageView) {
        ...
        do {
            try data = Data.init(contentsOf: url, options:.dataReadingMapped)
    ...
    @objc func handleDefocusBySwipeGesture(_ gesture: UISwipeGestureRecognizer) {
        ...
        focusViewController!.defocusWillStart()
        offset = (gesture.direction == UISwipeGestureRecognizer.Direction.up ? -kSwipeOffset : kSwipeOffset)
